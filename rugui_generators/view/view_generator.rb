class ViewGenerator < RubiGen::Base
  ...
  def manifest
    record do |m|
      # app/views
      m.template "view.erb", "app/views/#{@name.downcase}_view.rb", :assigns => { :view_name => @name.capitalize }
      # app/views/helpers
...
EOS
    ...
    def option_phrase_for(toplevel)
      "Create the glade file using #{toplevel} template as toplevel."
    end
